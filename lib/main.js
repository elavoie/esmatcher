...
function createMatcher(parentMatcher, rules, options) {
    ...
    function genResult(subresult) {
        for (var property in subresult) {
            ...
            if (typeof propertyValue === "string") {
                ...
                genCode(property + ": functions[" + functionNb++ + "](" + paramsWithoutMatch.join(",")  + ",WRAP_MATCH(match)),");
            } else {
                throw "Invalid pattern property type '" + (typeof propertyValue) + "' when parsing property '" + property + "'";
            }
        }
    ...
    genCode("var WRAP_MATCH = function (match) { return function (ast) { return match(ast,match); }; };");
    genCode.decIndent();
    ...
    for (var i = 0; i < rules.length; i += 2) {
        ...
        if (typeof result === "object") {
            ...
            genCode("return functions[" + functionNb++ + "](" + currentNodeName + ",WRAP_MATCH(match));");
        }
